...
tableFitDist <- function(data, ane = NA){
  ...
  if (class(data) != "windata") stop("The data isn't corresponded with the class 'windata'.")
  ...
  #Building the table
  t <- data.frame(cbind(Parameter1 = c(paste("k=", round(K, digits = 4)), paste("alpha=", round(alpha, digits = 4)), paste("m=", round(m, digits = 4))), 

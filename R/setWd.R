#' @title Conversion of dataframe in \code{windata} class object
#' 
#' @description
#' Shape a dataframe in a class \code{windata} object
#'  
#' @param data a dataframe to be converted
#' @param name name
#' @param date.var name of the variable with the dates of measurements
#' @param date.format date format in data: \code{YYYYMMDD}, \code{YYYY-MM-DD}, \code{YYYY.MM.DD} or \code{DD/MM/YYYY}     
#' @param time.var name of the variable with times of measurements
#' @param time.format time format in data: \code{HHMM}, \code{HHMMSS}, \code{HH:MM}, \code{HH:MM:SS}, \code{HH.MM} or \code{HH.MM.SS}
#' @param ane.names vector with names of the anemometers.
#' @param ane.height vector with the heights of the anemometers in meters. Length of this vector must be the same of ane.names.
#' @param speed.ave.var vector with the names of the variables that contains the average speeds measures. Length of this vector must be the same of ane.names.
#' @param speed.min.var vector with the names of the variables that contains the minimum measures. Length of this vector must be the same of ane.names.
#' @param speed.max.var vector with the names of the variables that contains the maximum measures. Length of this vector must be the same of ane.names.
#' @param speed.sd.var vector with the names of the variables that contains the standar desviation measures. Length of this vector must be the same of ane.names.
#' @param speed.unit wind speed unit. By the time, only 'm/s' can be used.
#' @param dir.var vector with the names of the variables that contains the wind direction measures
#' @param dir.unit wind directions unit. By the time, only 'deg' can be used.
#' @param temp.var name of the variable that contains the temperature measures.
#' @param temp.unit temperature unit.
#' @param pres.var name of the variable that contains the pressure measures.
#' @param pres.unit pressure unit.
#' @param NA.values vector with the the values that must be identified as missing values.
#' 
#' @return Object of class \code{windata}.
#' 
#' @details
...
setWD <- function(data, name = NA, date.var, date.format = c("YYYYMMDD", "YYYY-MM-DD", "YYYY.MM.DD", 
    ...
    speed.max.var = NA, speed.sd.var = NA, speed.unit = "m/s", dir.var, dir.unit = "deg", 
    ...
    if (date.format == "YYYYMMDD") {
        result[["time"]] <- data.frame(year = as.numeric(substring(data[, date.var], 
            1, 4)), month = as.numeric(substring(data[, date.var], 5, 6)), day = as.numeric(substring(data[, 
            date.var], 7, 8)))
    } else if (date.format == "YYYY-MM-DD" | date.format == "YYYY.MM.DD") {
        result[["time"]] <- data.frame(year = as.numeric(substring(data[, date.var], 
            1, 4)), month = as.numeric(substring(data[, date.var], 6, 7)), day = as.numeric(substring(data[, 
            date.var], 9, 10)))
    } else if (date.format == "DD/MM/YYYY") {
        result[["time"]] <- data.frame(year = as.numeric(substring(data[, date.var], 
            7, 10)), month = as.numeric(substring(data[, date.var], 4, 5)), day = as.numeric(substring(data[, 
            date.var], 1, 2)))
    } else {
    ...
    if (time.format == "HHMM" | time.format == "HHMMSS") {
        ...
        result[["time"]]$hour <- as.numeric(substring(pref0(data[, time.var], 5), 
            1, 2))
        result[["time"]]$minute <- as.numeric(substring(pref0(data[, time.var], 
            5), 4, 5))
    } else {
    ...
    result[["time"]]$dt <- as.POSIXct(paste(result$time$year, ".", pref0(result$time$month, 

...
plotDistWd <- function(data, ane = NA) {
    ...
    # Estimating parameters for each distribution browser()
    ...
    dr <- rbind(data.frame(dist = "wei", speed = data, ajust = y.wei), data.frame(dist = "ga", 
        speed = data, ajust = y.ga), data.frame(dist = "ln", speed = data, ajust = y.ln))
    # dw <- rbind(data.frame(dist = 'wei', speed = data, ajust = y.wei)) dg <-
    # rbind(data.frame(dist = 'ga', speed = data, ajust = y.ga)) dl <-
    # rbind(data.frame(dist = 'ln', speed = data, ajust = y.ln))
    
    
    
    
    # p1 <- ggplot(dw, aes(x = speed)) + geom_histogram(aes(y = ..density..),
    # binwidth = .5, colour = 'black', fill = 'blue') + geom_line(aes(y = ajust),
    # size=0.8) p2 <- ggplot(dg, aes(x = speed)) + geom_histogram(aes(y =
    # ..density..), binwidth = .5, colour = 'black', fill = 'blue') +
    # geom_line(aes(y = ajust), size=0.8) p3 <- ggplot(dl, aes(x = speed)) +
    # geom_histogram(aes(y = ..density..), binwidth = .5, colour = 'black', fill =
    # 'blue') + geom_line(aes(y = ajust), size=0.8) multiplot(p1, p2, p3, cols=3,
    # names = NULL)
    
    # ggplot(dr, aes(x = speed)) + geom_histogram(aes(y = ..density..), binwidth =
    # .5, colour = 'black', fill = 'blue') + geom_line(aes(y = ajust), size=0.8) +
    ...
    for (i in 1:3) {
        print(ggplot(dr[dr$dist == distr[i], ], aes(x = speed)) + geom_histogram(aes(y = ..density..), 
            binwidth = 0.5, colour = "black", fill = "blue") + ggtitle(tittle[i]) + 
            geom_line(aes(y = ajust), colour = "red", size = 1) + annotate("text", 
            x = max(data) * 0.8, y = 0.1, label = paste(namep1[i], " = ", round(para1[i], 
                digits = 4))) + annotate("text", x = max(data) * 0.8, y = 0.09, 
            label = paste(namep2[i], " = ", round(para2[i], digits = 4))) + annotate("text", 
            x = max(data) * 0.8, y = 0.08, label = paste("Loglik =", round(lo[i], 
                digits = 4))), vp = vplayout(rep(1:3)[i], 1))
        
    }
    
    # QQ-plots graphics browser()
    ...
    print(ggplot(dr, aes(sample = speed)) + stat_qq(distribution = qweibull, dparams = params.wei), 
        vp = vplayout(1, 2))
    print(ggplot(dr, aes(sample = speed)) + stat_qq(distribution = qgamma, dparams = params.ga), 
        vp = vplayout(2, 2))
    print(ggplot(dr, aes(sample = speed)) + stat_qq(distribution = qlnorm, dparams = params.ln), 
        vp = vplayout(3, 2))
